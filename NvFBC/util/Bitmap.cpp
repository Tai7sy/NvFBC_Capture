...
bool SaveARGB10(const char *fileName, BYTE *data, int width, int height, int stride)
{
    bool result = false;
    if (!data)
        return result;
    ...
    for (int row = 0; row < height; ++row)
    {
        for (int col = 0; col < width; ++col)
        {
            ...
            output[outputIdx].red = GET_8BIT_R_FROM_ABGR10(input[inputIdx]);
            output[outputIdx].green = GET_8BIT_G_FROM_ABGR10(input[inputIdx]);
            output[outputIdx].blue = GET_8BIT_B_FROM_ABGR10(input[inputIdx]);
        }
    }
